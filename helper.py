...
def preparefeatureLabel(batch_graph, batch_label, batch_adjs):
    ...
    batch_node_feat = torch.zeros(batch_size, max_node_num, batch_graph[i].shape[1])
...
class Trainer(object):
    ...
    def train(self, sample, model):
        ...
        prob, pred, labels, loss = model.forward(node_feat, labels, adjs, masks)

        return prob, pred,labels,loss
...
class Evaluator(object):
    ...
    def eval_test(self, sample, model, graphcam_flag=False):
        ...
        if not graphcam_flag:
            ...
            prob, pred,labels,loss= model.forward(node_feat, labels, adjs, masks, graphcam_flag=graphcam_flag)
        return prob, pred,labels,loss