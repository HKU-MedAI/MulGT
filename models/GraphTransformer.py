...
class MulGT(nn.Module):
    def __init__(self, subtype_class, stage_class, input_dim=512, bag_loss='nll_surv', alpha_surv=0.0,
        ...
        self.subtype_proto = nn.Parameter(torch.zeros(1, self.phase2_node_num, self.embed_dim))
        self.stage_proto = nn.Parameter(torch.zeros(1, self.phase2_node_num, self.embed_dim))
    ...
    def forward(self, node_feat, subtype_labels, adj, mask, stage_labels):
        ...
        idx_list = random.sample(range(0, X_subtype.shape[1] - 1), k=100)
